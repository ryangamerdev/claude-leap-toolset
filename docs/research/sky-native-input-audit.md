...
## 2026-09-20 — Text conversion precedes key factory

part-0026.c0x10072a5a0 calls SAIVirtualKeyPress.keyPressesForString.0x10072a660 iterates returned keys into0x10071b150, whose keyCode/modifiers/string fields feed the events. Thus matching the flagsChanged sequence alone misses text-to-physical-key conversion. Native Leap23-character request emitted two a characters with code0/chunking; Sky inserted actual phrase. Leap now derives physical text keys from current Carbon layout, not a guessed US layout or private class dependency. Unsupported single-stroke Simulator characters refuse before text dispatch. Full IME/dead-key behavior not reproduced.
