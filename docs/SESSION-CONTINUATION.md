...
Installed SHA-256 `444130a908e54dff63cb2a618a60bde37c75121fc33713da4334e98ecf8521db`. Latest increment: named grouping/discovery supersedes the timeline-only candidate below. See artifacts/test-runs/20260920-timeline-filter/install.json for installed identity. New tools recording_group (start/resume/end after bind_project) and recording_sessions (groups/recordings inventory); interaction_timeline now supports group_id and excludes empty IDs. Source/installed skills synchronized. Seven focused tests pass; native verification pending restart. First inspect existing 11 captures/589 records using recording_sessions(project,view:recordings), then bind (additive v1→v2 migration), start a named Simulator group and run the historical delta/new-toggle cases below. Record group_id for explicit resume after a later restart; old session IDs stay unchanged and prior history stays ungrouped. Group changes close app capture epochs; no UI input; next observation attaches a new epoch. [Iteration](iterations/2026-09-20-logical-sessions.md).
