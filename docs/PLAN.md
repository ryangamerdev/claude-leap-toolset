...
1. Restart onto the Mac CGFloat-bounds correction. The prior native double-click was rejected before input, so pointer behavior remains untested. Use explicit mac_ax Simulator/iPad session. Sky field double-click/zoom/reset passed in the latest comparison; repeat with Leap from its own fresh screenshot and snapshot, then drag and verify the field—not the window—moved. Test background first, explicit foreground if it fails. Sky sidebar wheel/drag showed no confirmed movement in that trial; do not invent a baseline pass. Coordinate scrolling is newly exposed in the intent API. WDA remains optional; no more speculative rotation experiments.
