...
1. Restart onto full-page scroll-evidence correction and verify desktop sidebar returns movement_observed for down/up, then continue desktop coach/create/save/reopen and paired Blender without checkpoint handbacks. iPhone paired semantic navigation passed; Leap foreground drag scrolled filters. Sky gestures failed window targeting in that trial. Scroll-effect negative-case reporting and retained evidence retrieval passed native iPad testing after restart (see iterations/2026-09-20-scroll-evidence-native.md). Wheel effect remains unverified in both focus modes; explicit drag visibly scrolls. Positive automatic movement detection remains open and should be tested on a supported scroll area during the next paired Sky/Leap iPhone landscape navigation/scroll case, then desktop coach/create/save/reopen. Do not repeat unchanged iPad wheel trials. iPad field zoom/reset/pan already passed scoped visual acceptance; no blanket background certification. WDA remains optional; no speculative rotation experiments.
