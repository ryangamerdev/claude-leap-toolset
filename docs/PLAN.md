...
1. Restart onto editor-metadata correction, resume existing desktop New Play1 editor, create/save/reopen LEAP ACCEPTANCE 0920 - Slant Flat with route-library assignments and notes. Sky desktop repeat saved/reopened successfully; full-page detector passed both directions. Then repeat create/save/reopen on iPad with Sky and Leap, per latest user steering, before paired Blender. Test data may be changed freely and need not be restored. Use role-qualified selectors or observed IDs when optional text-area labels are unknown. No interim checkpoint handbacks.
