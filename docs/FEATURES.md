...
| CAM-04 | Simulator discovery: explicitly identify the open iPhone and iPad device windows and their Gameday instances; target one without silently switching the other. | PENDING | PENDING | OBS-09, ENV-05/06/08, VIS-03, KEY-03. Read actual current titles/device identities; do not assume previous device names or schemas. |
...
|---|---|---|---|---|
| DATA-15 | P0 | Structured fresh/historical UI queries with role, IDs, state, root/depth, fields and byte/count limits. | IMPLEMENTED | ui_to_text; native app filters/pagination/geometry checks pending. |
...
| DATA-17 | P1 | Independent snapshot comparisons with completeness metadata and evidence references. | IMPLEMENTED | ui_diff; durable window identity and legacy metadata limitations remain. |
